#!/bin/bash
# Claude Code `Stop` hook: snapshot the workspace as a git commit after each
# turn that changed anything. Mirror of the Cursor kit's stop hook.
...
# the status with untracked files. Exclude rules only affect untracked
# files, so deliberately tracked paths are never dropped from the journal.
...
[ -e ".claude-git-safety-off" ] && exit 0
...
printf '%s\n' "$input" >> "$gitdir/claude-git-safety-payloads.jsonl" 2>/dev/null
...
sess=$(printf '%s' "$input" | /usr/bin/python3 -c '
import json, sys
try:
...
git commit -q --no-verify \
  -m "wip(claude): turn snapshot $(date +%Y-%m-%dT%H:%M:%S)${sess:+ [sess $sess]}"
